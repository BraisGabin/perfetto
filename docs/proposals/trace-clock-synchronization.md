...
- **Proto Traces**: Traces using Perfetto's native protobuf format, which contain
  rich metadata including explicit clock synchronization information.

- **ZIP/TAR Archives**: Compressed archive formats that can contain multiple trace
  files, requiring special handling to process each contained trace appropriately.
...
3. **Missing Clock Metadata Handling**: No way to specify the actual clock
   ...
   traces).
4. **No User Feedback on Clock Issues**: Users have no visibility into
   potential clock domain mismatches or way to correct them.
...
3. **Graceful Degradation**: Drop timestamped events for traces lacking
   required clock metadata while preserving non-temporal data
...
1. **Pure clock domain conversion algorithms** (stable, testable graph algorithms)
2. **Multi-trace/multi-machine coordination policy** (heuristic-driven, evolving)

This makes it difficult to add multi-trace coordination without disrupting the core algorithms.
...
- [ ] Extract pure algorithm functions from ClockSynchronizer into ClockGraphEngine
...
- [ ] **Commit:** `tp: refactor ClockSynchronizer into algorithm and coordination layers`

#### Task 1.2: Add Multi-Trace Clock Statistics

**Files:** `src/trace_processor/storage/stats.h`
...
**Category 1 Parsers (Explicit Clocks):**
Update parsers that currently call `SetTraceTimeClock()` directly:
...
**Category 2 Parsers (Semi-explicit Clocks):**
Update parsers to fetch configured clock or use default:
...
**Category 3 Parsers (Non-explicit Clocks):**
Update parsers to require configuration or drop events:
...
**Trace File ID Integration:**
Update `ForwardingTraceParser` to pass trace file ID to parsers:
...
- [ ] **Commit:** `tp: implement sidecar JSON metadata for multi-trace clock configuration`
...
1. **Detection Phase**: Analyze uploaded traces and identify Category 2/3 traces needing configuration
2. **Configuration Phase**: Present clock configuration UI for traces requiring metadata
...
- **Trace Analysis Display**: Show detected trace categories and current clock assumptions
- **Primary Trace Selection**: Radio buttons to choose which trace defines global clock domain
- **Clock Domain Override**: Dropdown for Category 2 traces to override format defaults
- **Clock Domain Specification**: Dropdown for Category 3 traces to specify required clocks
- **Relative Offset Input**: Numeric input fields for time offsets between traces
...
- **Multi-trace explicit traces**: Continue to work as before (proto, perf, etc.)
...
out/linux_clang_release/perfetto_unittests --gtest_brief=1 --gtest_filter="MultiTrace.*"
